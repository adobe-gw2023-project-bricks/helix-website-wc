...
export class Brick extends HTMLElement {
  ...
  get config() {
    let config = null;
    if (!this.root.querySelectorAll) return config;
    this.root.querySelectorAll(':scope > div').forEach((row) => {
      if (row.children) {
  ...
  fillTemplateSlot(slotName, element) {
    this.shadowRoot.querySelector(`slot[name="${slotName}"]`).replaceWith(element);
  }

  querySelectorFromRoot(selector) {
